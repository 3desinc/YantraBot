...
# Reads the sensor data from MPU 6050 - Accelerometer
import time
...
def get_data():
  ...
  # Convert raw data of sensor (output) to "g" units, so divide by 9.8 m/s2 (get acc_x g's)
  acc_x = data[0] / 16384.0	# <13>
  acc_y = data[1] / 16384.0
  acc_z = data[2] / 16384.0
  # Covert tempraw data to celcius
  zero_point = -512 - (340 * 35)	# <14>
  temp = (data[3] - zero_point) / 340.0	# <15>
  # Convert raw data into velocity, units degrees/s
